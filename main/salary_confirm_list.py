...
from .salary_confirm import salary_confirm_event
from .Ui.Ui_salary_pay_list import Ui_Dialog


class salary_confirm_list_event(QDialog, Ui_Dialog):
    
    salaries = None
    salary_confirm = None
    
    def __init__(self, parent=None):
        super(salary_confirm_list_event, self).__init__(parent)
        self.setupUi(self)
        self.salaries = session.query(Salary).filter_by(confirm = 0).all()
        for i, salary in enumerate(self.salaries):
            ...
            button.setText('登记')
            button.clicked.connect(partial(self.open_salary_confirm, salary, count, totalcost))
            self.table.setCellWidget(i, 4, button)
            
    def open_salary_confirm(self, salary, count, totalcost):
        self.salary_confirm = salary_confirm_event(salary, count, totalcost)
